...
export default function ProjectCard({ item }: { item: PublicProject }) {
  // Make sure to handle both upper/lowercase since source uses lowercase ("ongoing", "completed")
  ...
  const badge =
    statusStyles[item.status] ?? "border-white/15 bg-white/5 text-white/80";
  ...
  return (
    <article className="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-md transition hover:-translate-y-0.5 hover:border-white/20 hover:shadow-[0_18px_40px_rgba(0,0,0,0.25)]">
      <div className="flex items-start justify-between gap-3">
        <span
          className={`inline-flex rounded-full border px-3 py-1 text-[11px] font-semibold tracking-wide ${badge}`}
        >
